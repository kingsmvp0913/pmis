...
- 以繁體中文（臺灣）回覆；思考可用英文。實作前須明確列出核心假設；需求有歧義或存在多種合理解法時，停止實作，提出 2–3 種合理解讀或方案及其利弊，取得使用者確認後再決定，不得默默猜測。
- 只做解決需求所需、可驗證的最少程式碼；不得為未提出的彈性、擴充性或假設性錯誤情境建立抽象、欄位、邏輯或功能。
- 不得順手重構或整理相鄰、無關的程式碼、註解或格式；每一項修改都必須能直接對應需求，並維持既有程式碼風格。
...
- 開始工作前將需求轉為可驗證的成功條件與驗證方式；修復 Bug 時，優先以能重現商業問題的測試或可重現步驟建立基線，再修正並驗證未影響相關行為。每個重要步驟後說明已完成事項、驗證結果與待辦。規則或既有模式衝突時，選擇較新或較受測的一方並說明理由，不得混用。
