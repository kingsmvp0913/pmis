  ...
  async function render(content) {
    ...
    function renderDecision(projectId, projectName, data) {
      ...
      resultBox.appendChild(el('div', { class: 'card' }, [
        ...
        // 承包廠商在下游是挑施工日誌讀取器的鍵(vendor_id → vendors.name → 讀取器)。
        // 承辦人採用公告值後報表寫的是新廠商、系統仍用舊廠商解析日誌,兩者不一致卻毫無訊號;
        // 不自動改綁是 spec §5.3 明訂的行為,所以這裡只能把它講清楚。
        el('div', { class: 'hint' },
          '注意:承包廠商與主辦機關的裁決只影響監造報表的內容,不會改綁本工程的廠商/學校;'
          + '施工日誌仍以系統原本綁定的廠商挑選讀取器。若需改綁請另至工程管理頁調整。'),
