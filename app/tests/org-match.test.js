...
describe('normalizeOrgName', () => {
  ...
  // 那條規則救不到這個案例,需要另一條規則。
  test('「一」在兩個阿拉伯數字之間正規化為連字號(元長國小實測案例,無空白簡化版)', () => {
  ...
  // 真實 OCR 輸出本身含空白與全形括號,「一」與前後數字之間也夾著空白
  // (readKickoffReport 印出的原始值是「114 一 116」,不是「114一116」)。
  // 上一輪把這條正規化規則放在去空白**之前**,導致真實資料完全比對不到,
  // 手打的無空白字串測不出這個順序 bug——這支測試直接用真實字串,擋住
  // 同一個順序問題再次發生。
  test('真實 OCR 字串(含空白與全形括號)正規化後與決標公告相等', () => {
