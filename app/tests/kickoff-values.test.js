...
// ══════════════════════════════════════════════════════════════════
//  2026-08-15 逐份查證開工報告表時補的三條
// ══════════════════════════════════════════════════════════════════

describe('國字大寫金額的前綴與截斷', () => {
  // 義賢那份 OCR 讀到的是完整正確的「新臺幣貳佰柒拾參萬玖仟貳佰零陸元整」,
  // 卻因為第一個字是「新」而被「遇到不認得的字整串放棄」那條規則擋掉 → 整欄讀不到。
  test('幣別前綴要剝掉,不可以當成雜訊而整串放棄', () => {
    expect(parseMoney('新臺幣貳佰柒拾參萬玖仟貳佰零陸元整')).toBe(2739206);
    expect(parseMoney('新台幣貳佰肆拾伍萬陸仟元整')).toBe(2456000);
    expect(parseMoney('貳佰肆拾伍萬陸仟元整')).toBe(2456000);   // 沒前綴照舊
    expect(parseMoney('新臺幣3,122,168元')).toBe(3122168);      // 阿拉伯數字那條路不受影響
  });

  // 豐榮那份被 OCR 斷成「新臺幣參佰貳拾伍萬壹」+「仟柒佰陸拾玖元整」兩格。
  // 只拿到前半段時硬解會得到 3,250,001 —— 真值 3,251,769。那是個看起來完全合法、
  // 比對層與承辦人都看不出問題的錯金額,寧可回 null。
  // 判準是國字大寫的慣例:跳過的位數要用「零」補(…貳佰**零**陸),
  // 所以「萬」後面直接接一個數字又剛好是最後一個字 = 單位被切掉了。
  test('被切斷的大寫金額回 null,不硬解成看起來合法的錯數字', () => {
    expect(parseMoney('新臺幣參佰貳拾伍萬壹')).toBeNull();
    expect(parseMoney('新臺幣參佰貳拾伍萬壹仟柒佰陸拾玖元整')).toBe(3251769);
    // 沒有尾數的不算切斷(那就是整萬的金額)
    expect(parseMoney('新臺幣參佰貳拾伍萬')).toBe(3250000);
    // 「零」補位的合法尾數不可以被誤擋
    expect(parseMoney('壹萬零貳')).toBe(10002);
  });
});

// 義賢那份 OCR 讀出「120日暦天」——`暦` 是 `曆` 的異體字,NFKC 不折它。
// 不折的話 parseDuration 的緊鄰式判準對不上,**整個契約工期讀不到**,
// 而畫面上只顯示「工期缺漏」,指不出原因。
test('異體字「暦」要折成「曆」', () => {
  expect(parseDuration('120日暦天')).toEqual({ 天數: 120, 基準: '日曆天' });
  expect(parseDuration('120日曆天')).toEqual({ 天數: 120, 基準: '日曆天' });
  expect(parseDuration('機關通知日起90日暦天竣工')).toEqual({ 天數: 90, 基準: '日曆天' });
});
