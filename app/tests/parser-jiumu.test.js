...
test('PDF 新版欄寬仍能依表頭辨識本日完成金額', () => {
  const mk = (y, items) => ({ y, items: items.map(([x, w, s]) => ({ x, w, y, s })) });
  const rows = [
    mk(775.56, [[22.4, 11.5, '項次'], [110.8, 40.3, '工  程  項  目'], [214.2, 11.5, '單位'],
      [232.6, 82.9, '契 約 數 量 契  約  單  價 契'],
      [356.4, 82.8, '本日完成數量 累 計 完 成 數 量'],
      [441.4, 87.9, '本 日 完 成 金 額 累 計 完 成 金 額']]),
    mk(614.154, [[35.8, 3.8, '5'], [54.6, 99.9, '產品,施工測量,測量與放樣'],
      [216.1, 7.6, '式'], [250.4, 11.5, '1.0'], [280.3, 23, '10,200'],
      [326.9, 23, '10,200'], [373.3, 19.2, '0.500'], [415.7, 19.2, '0.500'],
      [459.5, 19.2, '5,100'], [505.7, 19.2, '5,100']]),
    mk(440.8734, [[54.6, 84.6, '累計(本日完成金額合計)'], [455.6, 23, '73,345']]),
  ];
  expect(mod._internal.parsePdfItemRows(rows)[0]).toMatchObject({
    項次: '5', 本日完成數量: 0.5, 累計完成數量: 0.5, 本日完成金額: 5100,
  });
});

test('PDF 分開的本日與累計金額表頭仍能切出本日金額', () => {
  const mk = (y, items) => ({ y, items: items.map(([x, w, s]) => ({ x, w, y, s })) });
  const rows = [
    mk(778.56, [[30.6, 11.5, '項次'], [118.9, 40.3, '工  程  項  目'], [222.4, 11.5, '單位'],
      [364.6, 82.8, '本日完成數量 累 計 完 成 數 量'],
      [449.5, 35.8, '本日完成金額'], [487.4, 26.8, '累 計 完 成']]),
    mk(465.6, [[42, 7.6, '伍'], [62.8, 23, '保險費'], [224.3, 7.6, '式'],
      [266.3, 3.8, '1'], [288.5, 23, '44,647'], [335, 23, '44,647'],
      [377.3, 19.2, '0.003'], [423.8, 19.2, '0.005'], [469.4, 11.5, '136'],
      [505.3, 11.5, '229']]),
