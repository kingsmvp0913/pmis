...
test('相同尾碼在不同大類下是不同項目，小寫與大寫中文數字不互轉', () => {
  const items = [
    item('1', 100, { 大類: '壹.一', 項目: '跑道整地' }),
    item('1', 200, { 大類: '壹.貳', 項目: '球場整地' }),
  ];
  expect(validateItems(items)).toEqual([]);
  const set = itemsToOperations(items).find((o) => o.type === 'setRange');
  expect(set.values.map((v) => v[0])).toEqual(['壹.一.1', '壹.貳.1']);
});
...
// 來源經費總表以大類列 + 裸項次表達階層，報表與資料鍵都必須保留組合後的完整路徑。
