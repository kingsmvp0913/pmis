...
function normalizeOrgName(v) {
  ...
  const s = String(v)
    ...
    // 全形空白也要清:PDF 抽取常挾帶
    .replace(/[\s　]/g, '')
    // 以下兩條「易誤讀字元」正規化,**必須放在去空白之後**:OCR 逐字掃描常在
    // 連字號/數字/中文字之間插入間距(元長國小實測的原始 OCR 值其實是
    // 「114 一 116」,一與前後數字中間都夾著空白,不是緊鄰的「114一116」)。
    // 若放在去空白之前,`(\d)一(?=\d)` 這類判斷前後字元的規則會因為中間隔著
    // 空白而永遠比對不到,規則等於形同虛設——這正是上一輪漏掉的順序問題:
    // 當時只用手打的無空白字串驗證,沒測出真實資料含空白會踩到的順序陷阱。
    //
    ...
    // (元長國小實測:OCR 把「114-116年」的半形連字號讀成「114 一 116年」)。
