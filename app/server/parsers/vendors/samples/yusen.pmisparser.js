 ...
 *
 * ⚠️ **vendorKey 待確認**:樣本檔內找不到廠商全名(第二聯的簽名欄是空白,
 * 進度分頁也沒有),此處依同批其他廠商的命名慣例推定為「玉森營造有限公司」。
 * registry 依此名稱查 vendors 表歸位,**名稱不符會被標記 unmatched 而裝不上**——
 * 安裝前務必與 vendors 表核對,不符就只改這個字串即可。
 * (同一份工程另有 `.docx` 版可能載明全名,但 filetypes 目前沒有 docx 支援。)
 *
 * ── 版面事實(實測)──
...
const META_VENDOR_KEY = '玉森營造有限公司';
