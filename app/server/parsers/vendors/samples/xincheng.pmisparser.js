 ...
 * +9~  明細:項次0 名稱1 單位7 契約數量8 本日完成數量11 累計完成數量13 單價19 累計金額20
 * ```
 *
...
const COL = {
  ...
  本日完成數量: 11, 累計完成數量: 13, 單價: 19, 累計完成金額: 20,
};
...
function parseItemRows(grid, start) {
  ...
  for (let i = start + OFFSET.明細起; i < grid.length; i++) {
    ...
    out.push({
      ...
      本日完成金額: null,          // 這家沒有本日金額欄(只有累計,見檔頭)
...
function selfTest() {
  ...
  put(9, [[0, '1'], [1, '安裝鋁擠型床組上下舖'], [7, '組'], [8, 18], [13, 0], [19, 18000], [20, 0]]);
  ...
  if (a.項次 !== '1' || a.單位 !== '組' || a.契約數量 !== 18 || a.契約單價 !== 18000) return false;
  if (a.本日完成金額 !== null) return false;
