 ...
 * 才有答案卷(同一份文件的文字層)。**真掃描件更差**:有雜訊、傾斜、印章、
 * 影印衰減。實測饒平 8/1-8/2 的真掃描件,第二聯五個表頭錨點裡「單位」
 * (直排兩個小字)OCR 根本沒偵測到,宜謙——表現最好的那一家——整份 throw。
 * 那是掃描解析度的天花板,不是參數調得動的(原稿約 200~300 DPI,render 拉高
 * 只會放大雜訊)。真掃描件的可用性見 `real-scan-viability.js`。
 *
