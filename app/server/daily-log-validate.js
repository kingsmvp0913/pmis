...
const WEATHER = new Set(['晴', '晴天', '陰', '陰天', '多雲', '雨', '雨天', '陣雨', '雷雨', '颱風', '霧', '雪', '晴時多雲', '多雲時晴', '陰時多雲', '陰雨', '雨陰', '大雨', '局部陣雨', '短暫陣雨', '細雨']);
...
// 於是費用項的項次變成純數字,B2/F1/C1 的降級與 B3 跳過分支就全部失效——
...
function validateDailyLog({ days = [], contract = [], project = {}, prior = {} } = {}) {
  ...
  for (const d of days) {
    ...
    for (const r of d.dailyRows || []) {
      ...
      if (!skippedCodes.has('B3') && 本日金額 != null && !isSubtotalRow(r) && !是費用項目) {
      ...
      // C1/C3 逐列範圍檢查。
      // C1 與 B2/F1 同樣要放過費用項目:那幾列的「完成數量」各家語意不一,
