 ...
 *
 * 不做 contacts/address/import——事務所只需要名稱,YAGNI。
 */
...
function registerRoutes(app) {
  app.get('/api/firms', verifyToken, async (req, res) => {
    try {
      const { rows } = await query('SELECT id, name, created_at FROM firms ORDER BY name');
      res.json(rows);
    } catch (err) {
  ...
  app.get('/api/firms/:id', verifyToken, async (req, res) => {
    try {
      const { rows } = await query('SELECT id, name, created_at FROM firms WHERE id = $1', [req.params.id]);
  ...
  app.post('/api/firms', verifyToken, async (req, res) => {
    try {
      ...
      const { rows } = await query(
        'INSERT INTO firms (name) VALUES ($1) RETURNING id, name, created_at', [name]
      );
  ...
  app.put('/api/firms/:id', verifyToken, async (req, res) => {
    try {
      ...
      if (dup[0]) return res.status(400).json({ error: '已有同名事務所' });
      const { rows } = await query(
        'UPDATE firms SET name = $1 WHERE id = $2 RETURNING id, name, created_at',
        [name, req.params.id]
      );
      if (!rows[0]) return res.status(404).json({ error: '事務所不存在' });
      res.json(rows[0]);
