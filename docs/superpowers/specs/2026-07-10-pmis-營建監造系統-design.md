...
|---|---|
| **登入** | 首次啟動導向建立管理員 |
| **廠商** | list + 搜尋;編輯:名稱 + 多聯絡人(勾主要);**批次匯入**(見 §9);詳細頁含**讀取檔安裝**區塊(見 §8.1) |
...
### 8.1 讀取檔安裝(使用者端,不碰 git/npm/終端機)

`gen-vendor-parser` skill 由我(開發端)產出後,一般使用者需能在自己電腦上的 PMIS 自行安裝該廠商讀取檔。

- **讀取檔封裝**:單一可攜檔 `<廠商鍵>.pmisparser.js`,檔頭 metadata 帶「廠商鍵、版本、目標欄位、內附 fixture」。
- **安裝入口**:**廠商詳細頁的「讀取檔」區塊**(非獨立頁),顯示狀態(未安裝 / 已安裝:版本、目標欄位、安裝時間)。
  - **「安裝讀取檔」按鈕** → 選檔上傳 → **驗證**(執行內附 fixture 測試 + 檢查檔頭廠商鍵是否對應**當前廠商**,避免裝錯家)→ 存入 `data/vendor-parsers/<廠商鍵>.pmisparser.js` → 註冊 `vendor_id → 讀取檔`。
  - 已安裝時可「**更新**」(換新版)/「**移除**」。
- **權限**:僅 admin 可安裝 / 更新 / 移除。
- **落地位置**:`data/vendor-parsers/`(**在程式碼樹外**),app 日後更新**不覆蓋**使用者已裝的讀取檔;執行時動態載入。
- **護欄**:fixture 未通過或廠商鍵不符 → 拒絕安裝並明確中文提示,不啟用壞檔。
