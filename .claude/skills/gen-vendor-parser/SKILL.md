...
使用者以截圖、錯誤清單或既有案件回報問題時，不能只修讀取器後就宣稱完成。先把問題
定位到下列其中一層，並在交付時逐一回報：

1. **施工日誌讀取結果**：原始日誌的欄位是否讀錯、漏讀或錯位。
2. **契約詳細價目表**：`contract_items` 是否已經以錯誤的來源／項次／單價建立。
3. **驗證規則或報表資料**：讀取值正確但 SP3 判斷、既有日報資料或報表公式仍錯。

原始日誌、原始發包經費總表與畫面錯誤三者至少要能對照兩者；手上三者都有時必須三者
交叉驗證。若程式修正不會回寫既有 `contract_items`／已產生報表，必須明講「程式已修」
與「既有資料仍待重建」是兩件事；沒有完成既有資料重建或取得明確不可重建的原因，不得
把該案件標示為已完成或歸檔。
...
2. **每份都試著開一次,分成三堆**:有文字層的 PDF / Excel / **無文字層的掃描件**。
   宜謙 31 份裡 14 份是掃描件、忠信整案都是。
   ⚠️ **掃描件不是「不能做」,是走另一條路**(`scanDays`,OCR items → 你的讀取器),
   但要先量三件事才知道值不值得——尤其**有沒有項次欄**是分水嶺。見下方「掃描件」。
   判斷「是不是掃描件」:`extractItems` 回來的 item 總數是 0。
...
5. 若來源同時提供「原契約／詳細價目表」分頁，必須以它驗證費用項與總額；不可只驗
   施工項目。項次可能用 `二三四…` 或 `貳參肆…`，兩種都要列入真實樣本或純函式回歸測試。
...
**① 項次怎麼給**。數一下「總明細列數 ÷ 天數」:
   ...
   - **要不要加大類前綴,看發包後經費總表怎麼編號**:石榴/饒平沒有中類、項次就是 1..N 與
     ...
     費用項(貳~陸)本身是頂層編號不加。
   - 副作用要知道:`是費用項目 = !/^\d+$/.test(項次)`,前綴或用名稱會讓 B2/F1/C1
     從硬錯降成軟警告，B3 則因費用比例無法與單價相乘而跳過(驗證層已補「也認費用名目」
     的後路,但仍要心裡有數)。
...
讀取器新增或修改後，`daily-log-source-locator.js` 必須能把統一 schema 欄位反查到該讀取器實際取值的位置。下載「廠商問題標註」時會直接使用這個位置在原檔畫框：

- Excel：`{ kind:'excel', sheet, address, field }`；合併格的 `address` 必須是完整合併範圍。
- PDF／OCR：`{ kind:'pdf', page, x, y, width, height, field }`。
- Word：`{ kind:'word', table, cell, field }`；`cell` 是該表格的實體儲存格序號，可正確處理不規則合併格。

**不得只搜尋與欄位值相同的文字後挑最近者。** 同頁、同列或同一活頁簿常有多個相同的「式」「1」「晴」。來源定位須用讀取器的 `parseAll` 做差分重播：逐一替換候選原始格／文字 item，再確認只有該候選會改變指定日期、項次與工程項目的輸出欄位。幾何、日期及名稱只能縮小候選範圍；若縮小後找不到因果來源，必須回查完整候選，不能直接猜位置。
...
PDF 的一個欄位可能被文字層拆成多個 item，甚至跨兩個 y 列（明禮項次 4 的工程名稱）。來源定位不能要求單一 item 包含完整解析值；應以名稱片段建立候選，逐一覆寫並重跑讀取器確認因果。同一頁有多個因果片段時合併其外框；跨頁或無法證明屬於同一欄位時才回報「未唯一定位」。
...
2. **列邊界 / 欄序規則**:
   - PDF/逐列型(金大):以「行首 token 是否為**項次 id**(中文大寫壹貳參… 或 1–2 位阿拉伯整數)」為列界;續行(名稱片段/單位/數字/`-`)併入當前列;再以「第一個單位 token」切「名稱 | 數字欄」。
     ...
     ⚠️ 續行合併前必須先辨識「總計／合計／簽章」等表尾並停止。展翔實測最後一項營業稅
     之後緊接「總計」與「簽章」，未先停止會把表尾接成項目名稱，導致每天一筆 E3 假警告。
...
### Excel
- **畫面上的日誌可能只是由選擇器驅動的單日檢視。** 銘佑龍井與阿奎新社檔都只顯示
  `內容!I6` 指定的一天，但 `內容!J:IV` 才是完整逐日矩陣，且後面日期列已先填滿公式。
  遇到這類檔案要讀底層矩陣，並以天氣、數量、人員、機具或紀錄等實際輸入判定已填日；
  不可只讀目前顯示頁，也不可因日期公式存在就把未來空白日全部收進來。fixture 必須斷言
  首日、目前顯示日、最後已填日與總天數。有些範本在中間空白區之後又出現預填天氣
  （阿奎新社第 154 欄後），這是未來範本值；從首日起的連續已填區遇到第一個完全空白日
  就應停止，不可跳過空白後繼續收遠端公式值。
- ⛔ **隱藏欄不要讀。** 那是廠商的工作區,不是他向機關申報的內容。
  新成實測:表格右邊 R~V 五欄**全部隱藏**,裝著 `S 累計完成數量 / T 單價 /
  U 累計完成金額(=S*T)`——**承辦人在畫面上根本看不到那個價格**。
  照讀的話,那套單價與發包經費總表 17 項裡有 14 項不同(逐項複價的合計倒是分毫不差,
  1,193,638),整案每天都會被 E6 擋下,而原因只是廠商自己都看不到的一欄。
  **可見表格沒有的欄位就回 null**,讓契約值以發包經費總表為準。
  ⚠️ 判斷方式:`XLSX.readFile(f,{cellStyles:true})` 之後看 `ws['!cols'][i].hidden`。
  **這件事逐格比對永遠看不出來**——值都在、也自洽,只是不該用。
...
```
scanCoverage(pdf, {ocr, extractItemsOcr})  → 這份涵蓋哪些日期(不需要讀取器)
scanDays(pdf, {ocr, extractItemsOcr, filetypes, parser}) → parseAll 的輸出
```
⚠️ **`scanDays` 是「OCR items → 廠商讀取器」**:它把 OCR 的框換算成與 `extractItems`
同形狀的 items 再餵給你的讀取器。所以**掃描件那一家的讀取器還是要寫**,
只是版面要拿 OCR items 去探勘(座標型的寫法完全一樣)。

**解析度一律 2200,不要跟著開工報告表調到 3400。** 密集表格的最佳點與開工報告表相反
(2200 86.8% / 2800 85.0% / 3400 84.4%,且 2800 以上開始把數字讀成別的數字)。

**OCR 很慢(4 頁 43 秒),第一件事就是把結果存成 JSON 快取**,之後離線迭代抽取規則。

#### 決定要不要做之前,先量這三件事

1. **涵蓋範圍**:`scanCoverage` 幾乎一定可用——表頭那一行字大、獨立、沒有格線干擾,
   鎮西 60 頁實測填報日期 100% 正確。**即使明細做不了,這個也值得跑**:把
   「這份讀不到」變成「這份是 6/1~6/30 共 30 天,要人工補」。
2. **有沒有項次欄**。這是**能不能做的分水嶺**,不是細節:
   - 有項次欄 → 名稱有 OCR 錯字也無所謂,SP3 靠項次對應(`contractByNo`)。
   - **沒有項次欄 → 只能用名稱當 key,而 OCR 的名稱錯字率高得驚人**。
     龍井(銘佑)實測一頁裡:`乙種施工团`(圍)、`速工带料`(連工帶料)、`给排水`、
     `折除`、`搞播`、`范圍`、`练材`、`黄任`——幾乎每個項目都帶 1~3 個錯字。
     這種情況下**讀取器寫得出來,但每一列都對不上契約表**,承辦人的「逐格確認」
     會變成「逐格重打」。要先解決**名稱模糊比對**(那是比對層的題目,不是讀取器)。
3. **數字格的實測對/錯/漏**。基座在 `data/parser-tools/ocr-ab/`,
   **一律量「對/錯/漏」不要量命中率**(後者會把「拿漏換錯」看成改善)。

#### 交付掃描件讀取器時要講清楚的代價

| 層 | 對 | 錯 | 漏 |
|---|---|---|---|
| item 層(OCR 框對不對得上文字層) | 86.8% | 0 | 13.2% |
| **dailyRows 層(6 份文件 296 格實測)** | **62.8%** | **1.7%** | **35.5%** |

⚠️ item 層那個 86.8% **不能拿來決定要不要開放**——中間隔著讀取器的表頭錨點與座標分欄,
逐家差異 0%~100%。
⚠️ 那 62.8% 還是拿**有文字層的檔案**走 OCR 路徑量的(只有那種檔案才有答案卷)。
真掃描件另有雜訊、傾斜、印章、影印衰減。
⚠️⚠️ **「錯的看不見」**:那 1.7% 的值本身自洽、累計也自洽,SP3 的 39 條驗證一條都
攔不住,會一路寫進每日施工紀錄 → 監造報表 → 估驗計價。**逐格確認是放行前提,不可省。**

#### 一個已經修掉的坑,別再踩

「真掃描件整份 throw」曾被記成事實,實際卡點不是辨識率:宜謙讀取器要求第二聯五個
表頭錨點全到,而「單位」是直排兩個小字、掃描件上 OCR 沒產生框——另外四個都讀到了,
單位欄的**值**也清清楚楚。改成**表頭找不到時從值回推欄界**之後,饒平六天全部解析出來,
本日完成數量對 6、錯 0、漏 0。**表頭抓不到不等於這份不能讀。**

細節與數據見記憶 `ocr-daily-log-findings`、`ocr-daily-log-rows-layer`。
...
5. **驗證(三道關卡,全部要過才算完成)**:
   ...
   b. **解析完整性 + 名稱形狀健檢**:`node scripts/check-parser.js <讀取器檔名> <樣本檔>`。
      ...
      並掃描項目名稱的形狀。原則上兩者必須為 0；但同一項跨多天每次都缺同一欄、且原始
      文件座標也證實空白時，應標為「來源格式未提供」並保留可見 skipped 說明，不能重複
      當作逐日硬錯，也不能憑空補值。其他部分缺漏一律回頭修讀取器。
   ...
   c. **跨層驗證**:同一支 `check-parser.js` 會把解析結果餵進 SP3 的 `validateDailyLog`(39 條規則)。
      **硬錯數 > 0 就要逐條確認是「廠商真的填錯」還是「讀取器讀錯」**,後者一律回頭修讀取器。
      ...
      `--contract <工程id>` 再跑一次**,讓 E3/E4/E5/E6 有外部基準；查詢前依 `getSQL`
      確認該工程的既有契約資料與原始來源是否一致。實測富森:E3 從 1679
      ...
      **前期資料是無文字層掃描件、永遠補不回來**的,也算已查明的結案理由(宜謙石榴案)。
      ...
      ⚠️ **名稱結尾為「小計」的列不可套 B3 量價核對**。小計列的完成金額可能是子項加總，
      而數量欄只代表其中一種完成比例；例如展翔「門窗與雜項工程小計」的累加金額
      376870，本來就不會等於累計數量 0.3 × 單價 97000。讀取器應照原欄位解析，
      由驗證層排除該列的 B3，不可為了讓數字相乘而改讀其他欄位。
   ...
   三關都過之後,才回報**確實對不到的欄位**(標紅,不靜默略過)。若發現是 SP2/SP3
   共用層缺陷，除非使用者明確要求端到端修復，先分開回報；有明確端到端授權時，可另開
   共用層修復，但必須補該層測試、驗證既有資料修復方式，不能把它混稱為「讀取器已完成」。
...
6. **交付與結案掃描**:讀取器進 repo `parsers/vendors/samples/`,`meta.vendorKey` = **廠商正式名稱**(中文)。安裝方式二擇一:①廠商詳細頁「安裝讀取檔」單家上傳(vendorKey 須等於該廠商名稱);②`POST /api/parsers/bulk` 多檔批次上傳,registry 依 `meta.vendorKey` 名稱自動歸位到同名廠商(對不到者標 unmatched)。兩者安裝時都會自動跑 selfTest 驗證。若任務來源是指定問題資料夾，結案前必須重新掃描該資料夾；只有「程式修正、既有資料狀態、原始檔驗證、待處理檔案」皆已逐項確認，才可將檔案移至已完成並回覆完成。
...
- 一次只做一家;預設不改其他階段的檔。**發現 SP3/SP2 的缺陷先記下來回報,不要順手改**；
  只有使用者明確要求端到端修復時，才可依「產生流程」第 5.c 的規則另行修正並驗證既有資料。
