...
本 skill 是**唯讀診斷工具**，不可用它直接 UPDATE／DELETE 修正資料。當畫面顯示施工
日誌的 E3/E4/E5/E6、契約項目或報表數字錯誤時，依序查：

1. 工程主檔與 `contract_items`（項次、名稱、單位、數量、單價、seq）。
2. 對應的原始附件／來源檔是否存在，以及其解析結果。
3. 施工日誌讀取結果與既有 daily records 是否一致。

回報必須明確區分「程式邏輯錯誤」「既有資料已錯」「來源檔本身錯」。若確認既有資料
需要重建，交接內容必須列出可重建的來源、目標工程與安全的既有功能路徑（例如重新建立
契約詳細價目表）；不得因本 skill 無寫入權限就把案件說成完成。若 v2 服務或連線不存在，
應如實標示為無法確認，不能改用未授權的資料庫寫入方式繞過。
